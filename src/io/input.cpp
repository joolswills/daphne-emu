						...
						if (find_word(cur_line.c_str(), sval1, cur_line))
						{
							if (find_word(cur_line.c_str(), sval2, cur_line))
							{
								if (find_word(cur_line.c_str(), sval3, cur_line))
								{
									...
									// fix button offset add 1 to keyboard mapping
...
void process_event(SDL_Event *event)
{
	...
	switch (event->type)
	{
		...
		case SDL_JOYBUTTONDOWN:
			//reset_idle(); 
			if ( ( joystick_buttons_map[event->jbutton.button] >= 0 ) && ( joystick_buttons_map[event->jbutton.button]  < SWITCH_COUNT ) )
			input_enable(joystick_buttons_map[event->jbutton.button]);
			break;
		case SDL_JOYBUTTONUP:
			if ( ( joystick_buttons_map[event->jbutton.button] >= 0 ) && ( joystick_buttons_map[event->jbutton.button]  < SWITCH_COUNT ) )
			input_disable(joystick_buttons_map[event->jbutton.button]);
			break;
